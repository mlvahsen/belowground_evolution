...
get_samples <- function(model, data, trait){
  ...
  M_all_sc <- M_all
  if ("ic_weight" %in% colnames(M_all)){
    M_all_sc[,"ic_weight"] <- scale(M_all[,"ic_weight"])
  }
  if ("ln_depth" %in% colnames(M_all)){
    M_all_sc[,"ln_depth"] <- scale(M_all[,"ln_depth"])
  }
  ...
  jm_mono = jags.model("main_code/Gaussian_RI_Model.txt", data=data, 
...
get_samples_pois <- function(model, data){# Get model matrices from linear models
  ...
  jm_mono = jags.model("main_code/Poisson_RI_Model.txt", data=data, 
                       n.adapt = n.adapt, inits=inits, n.chains=length(inits))
...
# to TRUE
run_models <- function(data, trait, diag_plot = F){
  # Set up data structure
  genotype <- as.numeric(droplevels(as.factor(data$genotype)))
  y <- data[,trait]
  nalpha <- length(unique(genotype))
  ...
  if(trait == "density"){
    jags_out <- get_samples_pois(model_template, data)
  }else{
    jags_out <- get_samples(model_template, data, trait)
  }
