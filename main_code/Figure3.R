# Belowground evolution Figure 3

# Figure caption: Provenance and cohort explains moderate variation in
# functional traits. (A) Using (generalized) linear mixed models with ecotype
# and age cohort as fixed effects, we decomposed observed trait variation into
# five categories: genotype, provenance, cohort, other (i.e. covariates in the
# model that accounted for exogenous variation due experimental set-up), and
# residual of each trait. (B) Differences in belowground biomass distribution
# with depth across provenance and age cohorts. Differences in the parameter,
# beta (inset), which were applied to the equation 1-beta^depth predict the
# cumulative proportion of belowground biomass with depth shown in the main
# figure (sensu Gale & Grigal 1987, Mozdzer et al. 2016). The vertical line at
# 95% cumulative probability delineates the depth at which 95% of belowground
# biomass is contained, which is a parameter in the marsh accretion model, The
# Cohort Marsh Equilibrium Model.
...
## Figure 3a - proportional variance explained by terms in linear model ####
